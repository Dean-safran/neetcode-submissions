...
"""
v is the number of courses, e is the number of prerequisite rules

Time : o(v + e)
We add all courses to both maps : o(v)
We then connect all edges : o(e)
Then topological sort processes each node by appending and popping 
off queue once, and processes all outgoing edges once  : o(v+e)

Space : o(v + e)
We have two maps that hold all nodes each, each edge is stored once
in each node it touches neighbor attribute, so two times overall : o(v + e)
"""
class Node :
