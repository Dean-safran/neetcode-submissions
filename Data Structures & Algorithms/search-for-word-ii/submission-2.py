...
class Solution:
    ...
    # the trie dominates space complexity, o(n) n being the number of 
    # chars in words

    # the total time complexity is o(n + m * 4 * 3^length_of_longest_word-1)
    # This means we visit every path possible on the board
    
    # n is the total amount of chars in words, we have to build the trie
    # creating nodes for each char, potentially

    # m is the total amount of blocks on the board, each starting block 
    # can recurse into 4 different subtrees, but in each subtree 
    # you can only recurse in three different directions (you can't
    # backtrack to a block you just visited). For each block 
    # we have 4 trees that are of size 3^l-1 (l being length of longest word)
    # The height of the tree is l-1 long, since we visited root, and the 
    # branching factor is 3. Hence we have (m * 4 * 3^l-1) in the time complexity 

    # we do not know what board will be given to us, worst case it could 
    # be that we have almost every prefix appear multiple paths, taking up 
    # almost every path, with a large enough board that that is plausible.
