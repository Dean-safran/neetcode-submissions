class Solution:
    ...
    # The pointers scan through every element twice, every 
    # time an element is scanned constant variable assignments 
    # and comparisons are made, the splicing at the end to 
    # return a substring starting at result[0] and ending at result[1]
    # is o(n), so over all time is o(n) and the pointers are o(1) space, but 
    # the hashmaps are o(m) space, the amount of unique chars in t
    ...
    def minWindow(self, s: str, t: str) -> str:
        ...
        for pointer2 in range(len(s)):
            ...
            while counter == required : 
                found = True
                if pointer2 - pointer1 < minDiff : 
                    result[0] = pointer1
                    result[1] = pointer2
                    minDiff = pointer2 - pointer1
