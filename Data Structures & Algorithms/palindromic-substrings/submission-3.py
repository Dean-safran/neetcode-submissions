...
o(n) scan for n char's in s, 
for total time o(n^2) with o(1) space for pointers
"""

"""
add special character sentinels to end to indicate end of string,
also add special chars in between to handle even length palindromes

treat every char as center, if you can expand left and right pointer
by 1 increase radius by 1 , once while loop finishes add
(radius + 1) // 2 to result 
"""
...
class Solution:
    def countSubstrings(self, s: str) -> int:
        ...
        result = 0
        for radius in p :
            # if palin radius is odd, you 
            # want to round UP to include center as a palin itself,
            #    this is the case because any ch not special 
            #    starts with one special char on each side, then adds
            #    two chars on each side every time following 
            #    that (# followed by ch)
            #    so it's always odd
            # if radius is even, just keep the rounding the same
            # this formula rounds odd numbers UP and even numbers 
            # stay the same under // 2
            result += (radius + 1) // 2 
